...
public class AdminDAO {
	...
	public MemberDTO memberLogin(String email, String pwd) throws Exception {
		Connection con = DBUtils.getConnection();
		String sql = "select member.*, floor(sysdate-member_expiredate) as bdate"
				+ " from member where member_email=? and member_pwd=?";
		PreparedStatement pstat = con.prepareStatement(sql);
	...
	public String getMemberPageNavi(int currentPageNo, String subject, String text) throws Exception {
		...
		for (int i = startNavi; i <= endNavi; i++) {
			if (currentPage == i) {
				sb.append("<li><a href='member.ao?currentPage=" + i + "' > <b>" + i + "</b></a></li>");
			} else {
				sb.append("<li><a href='member.ao?currentPage=" + i + "'>" + i + "</a></li>");
			}
	...
	public List<ReportDTO> getAllGroupReport(int type) throws Exception {
		Connection con = DBUtils.getConnection();
		String sql = "select * from create_group cg, report r where cg.group_name = r.report_calleegroup and report_type=?";
		PreparedStatement pstat = con.prepareStatement(sql);
		pstat.setInt(1, type);
		ResultSet rs = pstat.executeQuery();
		List<ReportDTO> list = new ArrayList<>();
